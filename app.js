...
const STU_STATUS_LABEL={active:'在读',transferred:'转出',suspended:'休学',graduated:'毕业',archived:'已归档'};
const STU_STATUS_TAG=s=>{const l=STU_STATUS_LABEL[s.status]||s.status||'在读';return s.status==='active'?`<span class="tag active">${l}</span>`:`<span class="tag archived">${l}</span>`};
...
function renderStudentDetail(d){
	...
	const statusTag=STU_STATUS_TAG(s);
...
function renderImportForm(){
	$('#import-body').innerHTML=`<div class="import-step">
		...
		<div id="import-file-area" style="display:none"><input type="file" id="import-file-input" accept=".csv,.xlsx"></div>
...
$('#register-form').addEventListener('submit',async e=>{e.preventDefault();try{const obj=Object.fromEntries(new FormData(e.target));if(obj.password!==obj.confirm)throw Error('两次输入的密码不一致');if(obj.role==='student'&&!obj.realName.trim())throw Error('学生注册需填写与班级名单一致的姓名');if(obj.role==='student'&&!obj.studentId)throw Error('学生注册需选择要绑定的学生档案');const body={username:obj.username,realName:obj.realName,password:obj.password,role:obj.role};if(obj.studentId)body.studentId=obj.studentId;const r=await api('/api/auth/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});setSession(r.token,r.user);e.target.reset();hideAuth();await refresh();toast('注册成功，已自动登录')}catch(err){toast(err.message)}});
$('#reg-role').addEventListener('change',async e=>{const isStu=e.target.value==='student';$('#reg-student-label').style.display=isStu?'':'none';if(isStu){try{const r=await api('/api/auth/students');$('#reg-student-sel').innerHTML='<option value="">请选择学生档案</option>'+r.students.map(s=>`<option value="${s.id}">${esc(s.name)}（${esc(s.className)}${s.studentNo?' '+s.studentNo:''}）</option>`).join('')}catch(e){toast('加载学生列表失败')}}});
...
const ATT_DEFAULT='present'; // 默认已到：老师只需改迟到/请假/缺勤/早退
function renderWsRecord(){
 ...
 $('#att-reset').onclick=async()=>{if(!confirm('把全班考勤重置为“全员已到”？'))return;try{await api(`/api/lessons/${ws.lesson.id}/attendance`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({records:ws.students.map(s=>({studentId:s.id,status:'present'}))})});await reloadWs();toast('已重置为全员已到')}catch(e){toast(e.message)}};
